...
def compute_dimrange(alpha, E_values, sample_sizes):
    dim_values = np.zeros(100)
    for n in range(1, 101):
        reg = linreg().fit(sample_sizes[:n].reshape((-1, 1)), E_values[:n])
        dim_values[n-1] = alpha / (1 - reg.coef_)
    return dim_values

def plot_dim_vs_samples(pts, alpha_range, fractal_name, true_dim, ptnum = 100):

    sample_sizes = np.linspace(3, log10(len(pts)), 100)
    E_values = [np.zeros(ptnum) for _ in range(len(alpha_range))]

    for ind, intervals in enumerate(get_distances(pts)):
        for ind1, alpha in enumerate(alpha_range):
    ...
    for alpha, E_values in zip(alpha_range, E_values):
        dim_estimates = compute_dimrange(alpha, E_values, sample_sizes)
...
alpha_ranges = [
    np.linspace(0.001, 2 - 0.001, alpha_num),
    np.linspace(0.001, log10(3)/log10(2) - 0.001, alpha_num),
    np.linspace(0.001, log10(8)/log10(3) - 0.001, alpha_num),
    np.linspace(0.001, 1 + log10(2)/log10(3) - 0.001, alpha_num),
    np.linspace(0.001, 2 - 0.001, alpha_num),
    np.linspace(0.001, 2 - 0.001, alpha_num)
]

for fractal_name, true_dim, arange in zip(sampler, dims, alpha_ranges):
    plot_dim_vs_samples(sampler[fractal_name](10**7), arange, fractal_name, true_dim, ptnum=200)
